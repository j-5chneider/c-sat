---
title: "Computer Supported Adaptive Teaching | Computer Assisted Adaptive Teaching"
author:
- affiliation: University of Tübingen       # When knitting to PDF
  email: juergen.schneider@uni-tuebingen.de               # delete this entire
  name: Jürgen Schneider                     # section and only use
  url: https://uni-tuebingen.de/de/175743           # - "your name, your email"
date: "`r Sys.Date()`"
output:
  html_document:
    code_folding: hide
...
* Aptitude-treatment-interaction (learning success under different treatments conditional on a group's aptitude)
  * e.g. expertise reversal effect
* Adaptive teaching
  * Definition: "Entscheidung für und Darbietung von Lehrangeboten, die basierend auf einer Diagnose von Lernendenmerkmalen angepasst werden."
    - Adaptive (umbrella term)
    - personalized = individualized (single learner)
    - differentiated (group of learners)
  * Adaptive teaching model
* Herausforderungen
  * es ist eine herausfordernde Aufgabe den Lernstand (bzw. Set an Schülermerkmale) zu diagnostizieren (besonders für alle SuS) [assess student model]
  * Welches ist das passende Angebot [decide on pedagogical model] für ein bestimmtes Set an Aptitudes?
  * Arbeitsaufwand bei der Generierung differenzierter Angebote [provide for learning process]
* hier können digitale Medien unterstützen adaptives Lernen zu ermöglichen.
* we want to systemize where computer can assist teachers to teach adaptively (Kommentar: Nutzen für die Praxis ist klar. Nutzen für die Forschung?)
  

![](../www/abb_loop model.png){width=100%}  
__Fig. 1:__ _Model based on Tetzlaff et al. (2020)_
...
1. On which __process__ (assess student model, decide on pedagogical model, provide for learning process) will computers give assistance?
    1. If on student model: On which __student aptitude__ (prior knowledge, learners path, affect & motivation, self-regulated learning) does computer assistance provide information?
    2. If on student model: Is it a dynamic or static model?
    3. If on pedagogical model: For which __aspect of pedagogy__ (content, method, media, quantity, quality) do computers give assistance in decision?
    4. If for learning process: For which __aspect of pedagogy__ (content, method, media, quantity, quality) do computers provide adaptive learning material?
2. On which __level__ (micro, meso, macro) is computer assistance adaptive?
3. To whom (teachers, students) will computers give assistance (locus of control)? [obsolet, wenn wir nur techer mediated betrachten?]
